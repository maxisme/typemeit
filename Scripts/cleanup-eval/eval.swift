...
@main struct Eval {
    ...
    static func markdown(_ results: [Result], passed: Int, total: Int, granted: Int, wished: Int) -> String {
        func quote(_ s: String) -> String { s.split(separator: "\n", omittingEmptySubsequences: false).map { "> " + $0 }.joined(separator: "\n") + "\n" }
        ...
        for (key, rs) in groups {
            ...
            for r in rs {
                ...
                out += "\n### \(mark) · \(r.input)\n\n"
                if r.screen != nil { out += "screen terms: \(r.terms.joined(separator: ", "))\n\n" }
                out += "expected\n" + quote(r.expected[0])
                for (alt, why) in zip(r.expected.dropFirst(), r.whys) { out += "\nor, \(why)\n" + quote(alt) }
                out += "\ngot\n" + quote(r.output)
                if let b = r.withoutScreen, b != r.output { out += "\nwithout the screen\n" + quote(b) }
            }
        }
        return out
    }
