...
# dense wisp into a thin cloud, left to right, on the site's paper, with the
# wordmark in DM Mono. The frames are the app's own shader (web/puff.glsl.js)
# rendered by headless Chrome through Scripts/og/render.html, since a browser
# is the only thing here that can run it. The PNG is 2x (2400 x 1260) so it
# stays sharp on Retina previews; it is mostly flat paper, so lossless is small.
...
# x is a fraction of the width; the rest are the shader's inputs. Density is
# eased down as the puff grows so the small one reads solid and the large one
# as the same smoke spread thin.
