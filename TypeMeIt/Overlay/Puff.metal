...
static half4 render(float2 position, float2 size, float time, float expansion, float trail, float flow, float disperse, half4 tint) {
    ...
    half a = half(alpha * (1.0 - disperse) * (1.0 - disperse)) * tint.a;
...
[[stitchable]] half4 puff(float2 position, half4 color, float2 size, float time, float expansion, float trail, float flow, float disperse, half4 tint) {
    return smoke::render(position, size, time, expansion, trail, flow, disperse, tint);
}
