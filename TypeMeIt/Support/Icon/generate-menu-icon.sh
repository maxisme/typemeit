...
# MenuBarIcon.swift tints and composes at run time.
...
mkdir -p "$(dirname "$OUT_OUTLINE")" "$(dirname "$OUT_ARCS")"
for pair in "outline.png:$OUT_OUTLINE" "arcs.png:$OUT_ARCS"; do
...
for dir in "$(dirname "$OUT_OUTLINE")" "$(dirname "$OUT_ARCS")"; do
