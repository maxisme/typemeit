...
Debug builds are a separate app, `TypeMeIt Dev` (bundle id `it.typeme.typemeit.dev`),
signed with the Developer ID certificate. Run it straight from DerivedData; never copy
a build over `/Applications/TypeMeIt.app`, which is the user's release install.
...
open "build/dd/Build/Products/Debug/TypeMeIt Dev.app"
```
...
settings item. `open -a "TypeMeIt Dev"` on a running instance also reopens it.
